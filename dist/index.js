...
async function getConsultationRequests(limit = 50, offset = 0) {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot get consultation requests: database not available");
    return [];
  }
  const result = await db.select().from(consultationRequests).orderBy((t2) => t2.createdAt).limit(limit).offset(offset);
  return result;
}
async function updateConsultationStatus(id, status) {
  ...
  const result = await db.update(consultationRequests).set({ status }).where(eq(consultationRequests.id, id));
  return result;
}
...
async function sendViaAwsSES(options) {
  try {
...
function getConsultationNotificationEmail(data) {
  return {
    to: "adam@ceyrva.com",
...
function getClientConfirmationEmail(data) {
  return {
    to: data.email,
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="border-top: 1px solid #ddd; padding-top: 20px; margin-top: 30px; color: #999; font-size: 12px;">
          <p>\xA9 2026 Ceyrva. All rights reserved.</p>
        </div>
      </div>
    `,
    text: `
...
\xA9 2026 Ceyrva. All rights reserved.
    `
  };
}
...
var MAX_SUBMISSIONS_PER_HOUR = 3;
...
function getRemainingSubmissions(ipAddress) {
  const now = Date.now();
  const entry = rateLimitStore.get(ipAddress);
...
setInterval(cleanupExpiredEntries, 10 * 60 * 1e3);
...
var appRouter = router({
  ...
  consultation: router({
    submit: publicProcedure.input(z2.object({
      ...
      company: z2.string().min(2),
      email: z2.string().email(),
      phone: z2.string().min(10),
      message: z2.string().min(10)
    })).mutation(async ({ input, ctx }) => {
      try {
        const clientIp = ctx.req.headers["x-forwarded-for"]?.split(",")[0] || ctx.req.socket?.remoteAddress || "unknown";
    ...
    updateStatus: protectedProcedure.input(z2.object({
      id: z2.number(),
      status: z2.enum(["new", "contacted", "archived"])
    })).mutation(async ({ input, ctx }) => {
      try {
